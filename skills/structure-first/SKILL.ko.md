...
- 함수 시그니처는 확정된 책임만 담는 최소 형태로 시작한다.
- 인자 추가는 책임이 실제로 바뀔 때만 허용한다(새 외부 입력, 의미 혼합, 경계 이동).
- 비즈니스 판단/계산 규칙은 atom/function 한 곳에서만 소유한다.
- 새 경로를 넣으면, 같은 변경에서 동등한 기존 경로를 제거/비활성화하거나 단계적 마이그레이션 계획(소유자, 종료 조건)을 함께 남긴다.
...
- 동일 판단 규칙의 다중 소유 금지
- 단계적 마이그레이션 계획(소유자, 종료 조건) 없이 신규/구경로 병행 유지 금지
...
- 부작용이 경계에 모여 있는가?
- 테스트가 계약 중심이며 간결한가?
- 인자 증가 정당성, 판단 규칙 단일 소유, 구경로 정리(또는 단계적 마이그레이션 계획)가 모두 확인되는가?
...
리팩터링 작업에서는 아래 1줄을 선택적으로 추가할 수 있음:

- `Refactor Check:` 인자 증가 근거 / 판단 소유 위치 / 구경로 상태(제거·비활성화·마이그레이션 계획)
