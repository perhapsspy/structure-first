...
프로젝트 문서와 문서 패키지를 읽기 쉽고, 재개하기 쉽고, 코드 작업의 기반으로 쓰기 좋게 만든다.
...
기존 문서가 이미 사실, 결정, 이동 경로를 소유한다면 다른 곳에 설명을 더하기 전에 그 문서를 고친다.
사용자 정정과 삭제 지시는 편집 입력으로 다룬다. 교체, 삭제, 보관/작업 문서로 격하, 또는 소유 문서 연결 중 하나로 처리한다.
제거할 내용을 경고문, 방어적 해설, 근거 없는 독자 가정, 중복 이동 경로로 현재 문서에 보존하지 않는다.
