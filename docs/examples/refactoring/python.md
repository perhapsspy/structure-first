...
## Problem
A single subscription charging function mixes lookup, pricing policy, payment, persistence, and notifications, so even small policy changes carry high regression risk.
...
def charge_subscription(user_id, plan_code, token, deps, coupon_code=None, trial=False):
    ...
    # Policy rules keep being appended over time
    if trial:
        amount = 0
...
- Readability impact: amount calculation rules are fixed as atoms, separating rule changes from I/O changes.
