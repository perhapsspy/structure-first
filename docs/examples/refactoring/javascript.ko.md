...
## Problem
대시보드 빌드 함수에서 조회/필터/정렬/뷰조립/캐시/분석 이벤트까지 한 번에 처리해, UI 요구사항 변경 시 사이드이펙트가 자주 발생하는 상태.
...
- 읽기 기준 변화: 필터/정렬/요약 계산과 외부 호출을 분리해 UI 요구사항 변경 시 영향 범위를 좁힐 수 있다.
